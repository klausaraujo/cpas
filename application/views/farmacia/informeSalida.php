...
* {
    ...
    <body>
      ...
      <h4 style="margin:0 0 10px 0;font-size:17px;text-decoration:underline;text-align:center;font-wight: bold;">
        <strong>AUTORIZACIÓN DE DESPLAZAMIENTO DE RECURSOS ESTRATEGICOS EN SALUD </strong>
      </h4>
      <br><br>
      <?php if($cabecera->num_rows()>0){ ?>
        <?php foreach($cabecera->result() as $row): ?>
          ...
          <div class="pdf__row">
            <h6>VI. Otras Actividades DIGERD:</h6>
            <p class="pdf__span">
              <?=$row->otrasactividadesdigerd?>
            </p>
          </div>
        <?php endforeach; ?>
      <?php } ?>	
      <?php if($detalle->num_rows()>0){ ?>
        <h6>VII.	Relación de bienes: </h6>
